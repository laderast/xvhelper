...
You will also need to specify the python path in RStudio (shown below).
...
data <- readr::read_csv("app46926_20230513090730.dataset.data.csv", show_col_types = FALSE)
head(data)
```
...
decoded_data <- data[1:5000,] |>
  decode_single(codings2) |>
  decode_multi_large_df(codings2) |>
  decode_column_names(codings2) 
```
...
```{r}
cohort1 <- readr::read_csv(glue::glue("{cohorts$name[1]}.data.csv"))
...
```{r}
codings2 <- codings |>
  dplyr::filter(ent_field %in% colnames(cohort1))

cohort1[1:10,] |>
  decode_single(codings2) |>
  decode_multi_purrr(codings2) |>
  decode_column_names(codings2)
```
